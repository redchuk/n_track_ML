...
def analyse_data(dots_df, nucl_masks, nucl_edges, pxsize, file):
    ...
    data_dist['serum'] = int(file.split('_')[1])
    data_dist['date'] = file.split('_')[0]
    data_dist['file'] = file[:-4]
...
def add_xy_deltas(df):
    ...
    data_a = data_a.iloc[:, 0:len(c_list)]
